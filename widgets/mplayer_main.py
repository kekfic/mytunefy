...
class MainWinPlayer(QObject, Ui_PlayerMainWindow):
    ...
    def __init__(self):
        ...
        if os.path.isfile('db_data/song_db'):
            ...
            self.category_list = self.get_playlist_in_db()
            songs_in_db = get_songs(self.category, option='')
    ...
    def select_list(self):
        ...
        self.songtracks = get_songs(self.category, option=item.text(), artist=True)
...
def get_songs(category, option, artist=False):
    ...
    """
    songs = []
    res = get_songs_from_db(category, option)
    if artist:
        for row in res:
            songs.append([row[4], row[1], row[3], row[5]])
    else:
        for row in res:
            songs.append(row[1])
